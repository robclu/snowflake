//==--- ripple/core/memory/pool_allocator.hpp -------------- -*- C++ -*- ---==//
//
//                            Ripple - Core
//
...
#ifndef RIPPLE_CORE_MEMORY_POOL_ALLOCATOR_HPP
#define RIPPLE_CORE_MEMORY_POOL_ALLOCATOR_HPP
...
class Freelist {
 ...
 private:
  ...
  static auto initialize(void* start, void* end) -> Node* {
    ...
    void* const second = align(offset(first, element_size), alignment);
    ...
    for (size_t i = 1; i < elements; ++i) {
      Node* next    = static_cast<Node*>(offset(current, size));
      current->next = next;
      current       = next;
    }
    assert(
...
/// compare the performance against other allocators with a locking policy.
...
class ThreadSafeFreelist {
  ...
  /// atomic and use it with relaxed ordering.
  struct Node {
 ...
 public:
  ...
  ThreadSafeFreelist(
    ...
    assert(current < end);
    assert(offset(current, size) <= end);
    current->next = nullptr;
...
#endif // RIPPLE_CORE_MEMORY_POOL_ALLOCATOR_HPP
