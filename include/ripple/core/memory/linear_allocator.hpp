//==--- ripple/core/memory/linear_allocator.hpp ------------ -*- C++ -*- ---==//
//
//                            Ripple - Core
//
...
#ifndef RIPPLE_CORE_MEMORY_LINEAR_ALLOCATOR_HPP
#define RIPPLE_CORE_MEMORY_LINEAR_ALLOCATOR_HPP
...
/// This types allocates data linearly from a provided arena. While it provides
/// an interface for freeing an individual element, it's an empty function, and
/// the allocator only allows resetting all allocations from the pool.
class LinearAllocator {
 public:
  ...
  : _begin(being), _size(uintptr_t(end) - uintptr_t(begin)) {}
  ...
  auto alloc(size_t size, size_t alignment) -> void* {
    ...
    void* const ptr     = align(current(), alignment);
    void* const curr    = offset(p, size);
 ...
 private:
  ...
  [[nodiscard]] auto current() const noexcept -> void* {
    return memory::offset(_begin, _current);
  }
  ...
  [[no_discard]] auto end() const noexcept -> void* {
    return memory::offset(_begin, _size);
  }
